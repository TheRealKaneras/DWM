package com.softetch.dwm.common.entity.layers;
...
@OnlyIn(Dist.CLIENT)
public class LeatherBroakirArmorLayer extends LayerRenderer<BroakirEntity, HorseModel<BroakirEntity>> {
   private final HorseModel<BroakirEntity> field_215341_a = new HorseModel<>(0.1F);

   public LeatherBroakirArmorLayer(IEntityRenderer<BroakirEntity, HorseModel<BroakirEntity>> model) {
      super(model);
   }

   public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, BroakirEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
