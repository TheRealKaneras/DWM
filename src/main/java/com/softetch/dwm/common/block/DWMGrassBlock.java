...
public class DWMGrassBlock extends Block {
    ...
    public void tick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
        if (!func_220257_b(state, worldIn, pos)) {
            ...
            worldIn.setBlockState(pos, DWMItems.GALLIFREY_DIRT.getDefaultState());
        } else {
            if (worldIn.getLight(pos.up()) >= 9) {
                ...
                for(int i = 0; i < 4; ++i) {
                    ...
                    if (worldIn.getBlockState(blockpos).getBlock() == DWMItems.GALLIFREY_DIRT && func_220256_c(blockstate, worldIn, blockpos)) {
