...
public class ItemTwoDis extends Item {
    ...
    public boolean itemInteractionForEntity(ItemStack stack, PlayerEntity player, LivingEntity target, Hand hand) {
        if(target.getPersistentData().contains(DWMNBTTags.SCALE_2D)){
            if(target.getPersistentData().getFloat(DWMNBTTags.SCALE_2D) <= 0.05f) {
                target.getPersistentData().putFloat(DWMNBTTags.SCALE_2D, 1.0f);
            } else {
                target.getPersistentData().putFloat(DWMNBTTags.SCALE_2D, target.getPersistentData().getFloat(DWMNBTTags.SCALE_2D) - 0.05f);
            }
        } else {
            target.getPersistentData().putFloat(DWMNBTTags.SCALE_2D, 0.95f);
        }
        return true;
    }
}
