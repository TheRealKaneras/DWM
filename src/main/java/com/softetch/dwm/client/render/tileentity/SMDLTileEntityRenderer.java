...
import com.softetch.dwm.client.model.smdl.SMDLModelEnvironment;
...
public class SMDLTileEntityRenderer<T extends TileEntity> extends TileEntityRenderer<T> {
    private Model model;
    private ResourceLocation texture;

    public SMDLTileEntityRenderer(TileEntityRendererDispatcher rendererDispatcherIn, SMDLModel model, ResourceLocation texture) {
        super(rendererDispatcherIn);
        this.model = model;
        this.texture = texture;
    }

    public SMDLTileEntityRenderer(TileEntityRendererDispatcher rendererDispatcher, SMDLModelEnvironment environment) {
        this(rendererDispatcher, environment.getModel(), environment.getDefaultTexture());
    }
    ...
    public void render(T tileEntity, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay) {
        ...
        if (texture == null) {
            ...
            this.texture = MissingTextureSprite.getLocation();
        }
        matrixStack.push();
